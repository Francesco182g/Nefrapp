...
public class GestioneMedico extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      if (operazione.equals("visualizzaProfilo")) {
        ArrayList<Paziente> pazientiSeguiti = 
            PazienteModel.getPazientiSeguiti(medico.getCodiceFiscale());
      ...
      if (operazione.equals("modifica")) {
        request.setAttribute("notifica", "Modifica effettuata con successo"); 
        // Nel caso in cui la modifica non
        ...
        if (tipo != null && tipo.equals("asincrona")) {
          ...
          request.setAttribute("pazientiSeguiti", pazientiSeguiti);
          dispatcher =
              getServletContext().getRequestDispatcher("/listaPazientiView.jsp"); 
          // pazienti
          dispatcher.forward(request, response);
          return;
        }
        ...
        if (!response.isCommitted()) {
          ...
          // Lascio "Disattivato" perchè non so cosa va a cambiare
        }
        return;
   ...
   * 
   * @param request usata per ottenere i parametri
   * @param response usata per il redirect
   ...
   * @return
   */
  private boolean validazione(String codiceFiscale, String nome, String cognome, 
      String sesso, String email, String residenza, String luogoDiNascita, String dataDiNascita, 
      String password, String confermaPsw) {
